...
#   ha aplicado un coeficiente debido a la superficie del sensor que contacta con el musculo
    ...
    for data in data_list:
        ...
        data_aux['MFHB'] = ((data_aux['Sensor_17'] + data_aux['Sensor_18'] + data_aux['Sensor_21'] +
                            data_aux['Sensor_22'] * 0.3 + data_aux['Sensor_23'] + data_aux['Sensor_25'] * 0.75 +
...
#    dataframe_original  = dataframe_original.drop(columns=['groups', 'Gait_event'])
    ...
    dataframe_to_group  = dataframe.drop(columns=['Timestamp_init', 'Timestamp_end','Date_init','Date_end',
                                                  'Source','Subject','experiment','Frame','MFHB','MINO',
                                                  'MDIM','MFDB'])
...
def automata_segmentativo(dataframe):
    ...
    for i in range(len(dataframe)):
        if dataframe.iloc[i].Gait_event == trans[0] and current_state == states[0]:
        ...
        elif dataframe.iloc[i].Gait_event == trans[2] and current_state == states[2]:
            ...
            step_dataframe['step'] = step_counter
            
            dataframes_step_list.append(step_dataframe)
...
def step_tokenization(dataframes_step_list):
    ...
    dataframe_resume_steps = pd.DataFrame(columns=['Timestamp_init', 'Timestamp_end', 'Step', 'Mean_MFHB',
...
def sym_secuencial_analysis(dataframe_steps_l, dataframe_steps_r):
    ...
    if dataframe_steps_l['step'].max() > dataframe_steps_r['step'].max():
        steps = dataframe_steps_r['step'].max()
    else:
        steps = dataframe_steps_l['step'].max()
    
    sym_ind = []
    
    for step in range(1, steps + 1):
        dataframe_aux_l = dataframe_steps_l[dataframe_steps_l.step == step]
        dataframe_aux_r = dataframe_steps_r[dataframe_steps_r.step == step]
        
        dataframe_aux_l = dataframe_aux_l.drop(columns=['Timestamp_init', 'Timestamp_end', 'Date_init', 'Date_end', 'Source',
                                                        ...
                                                        'Sensor_32', 'Frame', 'Subject', 'experiment',
                                                        'groups', 'Gait_event', 'step'])
        
        dataframe_aux_r = dataframe_aux_r.drop(columns=['Timestamp_init', 'Timestamp_end', 'Date_init', 'Date_end', 'Source',
                                                        'Sensor_1', 'Sensor_2', 'Sensor_3', 'Sensor_4', 'Sensor_5', 'Sensor_6',
                                                        'Sensor_7', 'Sensor_8', 'Sensor_9', 'Sensor_10', 'Sensor_11',
                                                        'Sensor_12', 'Sensor_13', 'Sensor_14', 'Sensor_15', 'Sensor_16',
                                                        'Sensor_17', 'Sensor_18', 'Sensor_19', 'Sensor_20', 'Sensor_21',
        ...
        dataframe_aux_l = utils.normalize_data(dataframe_aux_l)
        dataframe_aux_r = utils.normalize_data(dataframe_aux_r)
        
        diff        = dataframe_aux_l.values - dataframe_aux_r.values
        diff_flatt  = [val for sublist in diff for val in sublist]
        
        diff_flatt_len = len(diff_flatt)
        sym_ind.append(sum(diff_flatt) / diff_flatt_len)
